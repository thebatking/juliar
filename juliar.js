...
function Juliar_code(){
    ...
    css.innerHTML += ".marquee{margin: 0 auto;overflow: hidden;white-space: nowrap; box-sizing: border-box;}";
        ...
        for(var i=0,j=juliar.index;i<j;i++){
                ...
                fileref.textContent =juliar.code[i];
...
function Juliar_pick(str) {
        ...
        for(var i = 0, len = juliar_core_module.length; i < len;++i) {
                if (typeof window["juliar_"+juliar.module[i]+"_"+command] === "function") {
                        return window["juliar_"+juliar.module[i]+"_"+command](str.substr(length).trim(),args);
                }
        }
...
function juliar_core_list(str,args){
        ...
        var list = "var listcat= document.getElementsByTagName(\"juliar_list_"+category+"\");var temp = document.createElement(\"li\");temp.innerHTML = \""+str+"\";if(listcat.previousElementSibling.nodeName != \"OL\"){";
...
function juliar_core_blink(str,args){
        var rate = args[0] || 500;
        var temp = juliar.length;
        juliar.code('var handler = function() {'+
