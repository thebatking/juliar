...
function Juliar_main(juliar){
	...
	this.importjss = function(str,args){
		if(str.trim() === "") return "<juliar_error>You Did not specify what to import! Cannot Continue to Import</juliar_error>";
			...
			fileref.onchange = function(evt)
			{
				...
				reader.onload = function(e){
					...
					if(f.name.slice(index) == ".jss"){
						var css = document.createElement("style");
						css.type = "text/css";
						css.innerHTML = juliar.parser(contents);
						document.body.appendChild(css);
						}else if(f.name.slice(index) == ".css"){
						var css = document.createElement("style");
						css.type = "text/css";
			...
			var repo = args[0] || true;  //CSS REPO :)
			var http = new XMLHttpRequest();
			str.indexOf("//") != -1 ? http.open("GET", str, !1): http.open("GET", "modules/"+str+".jss", !1);
			http.send();
			if(http.status!=200){
				if(repo==false)return "<juliar_alert>Failed to Load CSS '"+name+"'</juliar_alert>";
				http = new XMLHttpRequest();
				http.open("GET", repolink+name+"&css=", !1);
				http.send();
				if(http.status!=200) return "<juliar_alert>Failed to Load CSS '"+name+"' from Juliar repo</juliar_alert>";
			}
			var css = document.createElement("style");
			css.type = "text/css";
	...
	this.commands = function() { //List commands
		...
		while(i--){
			for( var x in juliar.modules[modules[i]]) {
				...
				temp.push(x);
				tempinfo.push((i>2)? " <em>>> IMPORTED from <strong>"+modules[i] +"</strong><span class='juliar_error'> level: "+(i-2)+"</span></em>"  : "");
				}
				}
				}
				var list = [];
