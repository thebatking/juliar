function Juliar(verbose) {
    this.verbose = verbose || 0;
        ...
        this.clearcss = function(){csscode="";return "";}
...
function Juliar_code(){
    ...
    css.innerHTML = "body{font-family: Tahoma, Geneva, sans-serif;background-repeat:no-repeat;background-size:cover;}";
        css.innerHTML += ".center{text-align:center;}.left{text-align:left}.right{text-align:right}";
        css.innerHTML += ".smaller{font-size:95%}.larger{font-size:105%}.subscript{vertical-align: sub;font-size: smaller;}.superscript{vertical-align: super;font-size: smaller;}";
...
function juliar_core_help(str) { //Opens Documentation for the commands
        if(str == "") return "Type \\*help  'command name' \\* to see help for the command";
        var index = juliar.index();
        juliar.code('var xmlhttp = new XMLHttpRequest(); ' +
        'xmlhttp.onreadystatechange=function() ' +
        '{'+
        'if (xmlhttp.readyState==4 && xmlhttp.status==200) '+
        '{'+
        'document.getElementsByTagName("help_'+index+'")[0].innerHTML = xmlhttp.responseText'+
        '}'+
        'else{'+
        'document.getElementsByTagName("help_'+index+'")[0].innerHTML = "No Such Command Exists in help documentation. Please Check Spelling of \\\"'+str+'\\\""'+
        '}'+
        '};'+
        'xmlhttp.open("GET", "http://www.juliar.elementfx.com/juliar/help?q='+str+'", !0);'+
        'xmlhttp.send();');
        return "<help_"+index+"></help_"+index+"><br>";
}
...
function juliar_core_commands() { //List commands
        var functions = "";     
        for( var x in window) {
                if(x.indexOf("juliar_") === 0 && typeof window[x] === "function") {
...
juliar_core_middle = juliar_core_center = function(str) {
