...
var worker_fn = function(e) 
{
	...
	function background(str){
		return '<img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" onload="document.body.style.backgroundColor = \''+str+'\'">';
	}
	...
	for (var juliar = juliars.length; juliar--;) {
		...
		while((m = str.indexOf("*",last))!= -1){
			if(str.charAt(m+1) == "" || str.charAt(m+1) == " " || str.charAt(m+1) == "*" || str.charAt(m+1) == "\n"){
				if(!--stack){
					sliced = str.slice(begin,m+1);
					mtjuliar[juliar][counter] = sliced;
					str = str.replace(sliced, "<juliar_"+juliar+"_"+counter+"></juliar_"+juliar+"_"+counter+">")
					counter++;
					m = begin-1;
				}
			}
		...
		for (var t = 0;t < mtjuliar[juliar].length; t++) {
			worker[juliar][t] = new Worker(window.URL.createObjectURL(blob));
			worker[juliar][t].onmessage =  function(e){
				juliars[this.juliar].innerHTML = juliars[this.juliar].innerHTML.replace("<juliar_"+this.juliar+"_"+this.t+"></juliar_"+this.juliar+"_"+this.t+">", e.data)
			}.bind({t: t,juliar:juliar});
			worker[juliar][t].postMessage(mtjuliar[juliar][t]);
		}
	}
	juliar_injectcss();
