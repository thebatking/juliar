...
class Juliar_web{
	constructor(juliar){
		...
		this.block = (blockContent,width = "inherit") => `<div class="juliar_block" style='width:${width}'>${blockContent}</div>`;
		...
		//Text Effects
		this.left = content => `<div class='juliar_left'>${content}</div>`;
		this.right = content => `<div class='juliar_right'>${content}</div>`;
		this.center = content => `<div class='juliar_center'>${content}</div>`;
		this.middle = content => `<div class='jular_middle'>${content}</div>`;
		this.bold = content => `<span class='juliar_bold'>${content}</span>`;
		this.italics = content => `<span class='juliar_italics'>${content}</span>`;
		this.crossout = content => `<span class='juliar_crossout'>${content}</span>`;
		this.overline = content => `<span class='juliar_overline'>${content}</span>`;
		this.subscript = content => `<span class='juliar_subscript'>${content}</span>`;
		this.superscript = content => `<span class='juliar_superscript'>${content}</span>`;
		this.underline = content => `<span class='juliar_underline'>${content}</span>`;
		...
		this.smaller = content => `<span class="juliar_smaller">${content}</span>`;
		this.larger = content => `<span class="juliar_larger">${content}</span>`;
		this.shrink = content => {
			var output = "", escaper = 0, counter = 0;
			for (var i = 0, length = content.length; i < length; ++i){
				if(content[i] == '<') escaper = 1;
				if(escaper === 0){ output += "<span class=\"juliar_smaller\">" + content[i]; counter++;}
				else{ output += content[i];}
				if(content[i] == '>') escaper = 0;
			}
		...
		this.grow = content => {
			...
			for (var i = 0, length = content.length; i < length; ++i){
				if(content[i] == '<') escaper = 1;
				if(escaper === 0){ output += "<span class=\"juliar_larger\">" + content[i]; counter++;}
				else{ output += content[i];}
				if(content[i] == '>') escaper = 0;
			}
			for (i = 0; i < counter; i++) output += "</span>";
		...
		this.header = str => "<span class=\"header\">"+str+"</span>";
		this.footer = str => "<span class=\"footer\">"+str+"</span>";
		this.banner = str => {
	...
	constructor(juliar){
				...
				jselector.addEventListener("click", function(e){
					...
					switch(classname){
						...
						case "juliar_min_btn":
							...
							if(el.style.opacity == 1){el.style.opacity=0;el.style.height=0;}
							else{el.style.opacity=1;el.style.height="auto";}
						break;
						case "commandlist":
							var el = e.target.parentNode.parentNode.parentNode.getElementsByClassName("foreground")[0];
		...
		var keyUp = e => {
			...
			if (keyCode === 13 && target.value != "") {
				...
				var now = new Date();
				temp.innerHTML = "<span class='juliar_close_btn'> &#10006;</span><span class='juliar_min_btn'> &boxminus; </span>"+
				"<span class='time'>"+("0"+now.getHours()).slice(-2)+ ":" + ("0"+now.getMinutes()).slice(-2) + ":" + ("0"+now.getSeconds()).slice(-2)+"</span>"+
				"<div class='commandused'>"+str+"</div><hr><div class='jcontent' style='opacity:1'>"+juliar.parser(str)+"</div>";
